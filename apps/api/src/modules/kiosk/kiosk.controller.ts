  ...
  @ApiOkResponse({
    schema: {
      ...
      properties: {
        ...
        consistencia: {
          type: 'object',
          required: ['atual', 'recorde', 'diasPorSemana', 'politica', 'semanas'],
          properties: {
            atual: { type: 'integer' },
            recorde: { type: 'integer' },
            ...
            semanas: {
              ...
              items: {
                type: 'object',
                required: ['inicio', 'fim', 'diasTreinados', 'status'],
                properties: {
                  inicio: { type: 'string', pattern: '^\\d{4}-\\d{2}-\\d{2}$' },
                  fim: { type: 'string', pattern: '^\\d{4}-\\d{2}-\\d{2}$' },
                  diasTreinados: { type: 'integer' },
                  status: {
                    type: 'string',
                    enum: ['QUALIFICADA', 'PERDIDA', 'PAUSADA', 'EM_ANDAMENTO'],
                  },
                },
              },
            },
