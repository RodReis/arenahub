...
- **Caractere invisível se escreve como escape Unicode (`​`), nunca literal** — em regex, em string de teste e em comentário. A lint `no-irregular-whitespace` recusa o literal, e com razão: é um byte que ninguém enxerga na revisão. Os blocos de código abaixo mostram alguns literais por legibilidade — converta ao escrever.
