...
**Última atualização:** 09/09/2026 *(F62 entregue — identidade visual do tenant e login por slug: `/{slug}/login` mostra logo, nome, missão e diferenciais da academia, e o SVG do ícone é sanitizado no upload, não na hora de servir. Slug desconhecido e tenant fora de operação devolvem a mesma resposta, para a tela de entrada não virar oráculo de quem é cliente. Antes: F61 entregue — Super Admin, ciclo de vida do tenant e elevação de suporte auditada; três defeitos achados na própria revisão, entre eles o OWNER que nasceria preso à primeira unidade e a elevação que empilhava sem encerrar. Antes: F60 entregue; `[FIX]` #272 — assinatura duplicada inflava a receita esperada do painel financeiro; `[INFRA]` #274 — tela de usuários: convidar, listar e aceitar convite, que destrava criar administrador sem `curl`; `[FIX]` #276 — o link do convite passa a ser URL completa, clicável e copiável; `[FIX]` #281 — senha do convite cai de 12 para 8 caracteres, por decisão do PI; `[INFRA]` #277 — o convite passa a sair por e-mail pelo Resend, e a tela diz quando não sai)*
...
| F63 | SPEC-063 | 7 | — | Plano SaaS (por aluno ativo/inativo ou fixo corrigido por índice) e contrato imutável | [`SPEC-063-plano-saas-e-contrato.md`](specs/SPEC-063-plano-saas-e-contrato.md) · [ADR-052](DECISIONS.md#adr-052) | [#286](https://github.com/RodReis/arenahub/issues/286) | 📋 **backlog** — spec `aprovada-pi` em 08/09/2026 |
