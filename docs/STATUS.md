...
| F58 | SPEC-058 | 1 | — | Implantação: API e painel na Railway, totem local, pré-produção | [`SPEC-058-implantacao-nuvem-e-totem-local.md`](specs/SPEC-058-implantacao-nuvem-e-totem-local.md) · [ADR-051](DECISIONS.md#adr-051) | [#253](https://github.com/RodReis/arenahub/issues/253) | 🚧 **em andamento** — código mesclado ([#255](https://github.com/RodReis/arenahub/pull/255), com [#256](https://github.com/RodReis/arenahub/pull/256), [#257](https://github.com/RodReis/arenahub/pull/257) e [#258](https://github.com/RodReis/arenahub/pull/258) corrigindo o build de deploy): `REDIS_URL` obrigatória em produção, rate limit no login (10 tentativas erradas/min por IP+e-mail), porta 3210 fixa no `start` do kiosk, bootstrap de tenant idempotente. **Nuvem no ar em 02/09/2026:** Postgres, Redis e Bucket provisionados; **API respondendo** (`Nest application successfully started`, 55 migrations conferidas pelo pre-deploy) e **painel rodando**. Falta ligar o painel à borda (target port do domínio, §4 do runbook), o backup do Postgres (AC-8) e todo o §5–§8 do runbook — tenant real, base do Pacto, totem físico e as evidências. **Não é produção**: restrição 1 do ADR-029 segue em vigor |
