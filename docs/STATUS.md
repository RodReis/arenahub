...
**Última atualização:** 09/09/2026 *(F64 entregue — fatura da plataforma sobre o tenant: o ArenaHub passou a cobrar a academia. A contagem de alunos é congelada na emissão e nada a recalcula depois; a idempotência mora na chave única do banco, não num `if`. Inativo é contado por complemento, então um status novo cai nele sozinho. A prévia decompõe a conta em vez de mostrar o total, porque 66% dela vem de quem não treina. Antes: F63 entregue — plano SaaS e contrato do tenant)*
