...
jeito"*. Agora **20% dos alunos em risco ficam deliberadamente fora da fila** — decisão sua,
`M6-EXPERIMENT-01` —, e a diferença de permanência em 30 dias entre os dois braços é o que diz se o
CRM funciona.

**O grupo é verificável, não confiável.** A alocação sai de `sha256(semente ␟ aluno)`: qualquer
pessoa recalcula e confere contra o banco. Sortear e gravar funcionaria até a primeira pergunta
séria — *"como sabemos que ninguém mexeu na alocação depois de ver o resultado?"* —, cuja resposta
seria "confie na tabela".

**E o banco recusa a fraude, não o serviço.** Dois triggers: um impede mexer em semente ou fração
depois que o experimento começa, outro impede trocar o grupo de um aluno. Rodar → ver o resultado →
mudar a semente → rodar de novo é exatamente o cherry-picking que o aceite proíbe. Guarda em
serviço seria uma porta; correção manual e script de migração passam por fora dela.

⚠️ **Terceira fatia seguida em que um canário verde escondia buraco na suíte.** Remover a semente do
hash derrubava só o teste unitário — nenhum teste de integração usava duas sementes, então a
consequência real (o mesmo aluno preso no controle em todos os experimentos) nunca era exercitada.
O padrão já é regra: **guarda verde não prova nada quando o cenário não alcança a guarda.**
...
**F44 já entregue em 26/08**. A **F43 saiu do gate em 11/09/2026**, quando o PI mandou executá-la junto da F23 — e `apps/mobile` deixou de ser um `.gitkeep`.
...
| **2.5** | Design system: tokens, `packages/ui` e as três superfícies | **sem gate ativo** — o da F43 e o da F44 já abriram | F42–F44 | criado por **ADR-025**. **As três superfícies existem**: F44 (totem) em 26/08, F46 (painel) e **F43 (app) em 12/09/2026**, quando o PI priorizou o MVP 4 |
...
| **4** | Autosserviço: **app do aluno** (o totem saiu para o MVP 3.5) | APIs estáveis dos MVPs 1, 2 e 3 | F23–F29 | **em execução desde 11/09/2026**, quando o PI mandou fazer F43 e F23. A camada de UI saiu primeiro (**F43**, 12/09) e `apps/mobile` deixou de ser um `.gitkeep`; a **F23** — identidade e shell — é a próxima. **Slices 4.5 e 4.6 são executadas no MVP 3.5**; o texto e o aceite continuam no PRD MVP-04 §7, sem cópia |
...
|---|---|---|---|---|---|---|---|
| F76 | SPEC-076 | 1 | — | Convidados no plano: passe mensal com nome e CPF | [`SPEC-076-convidados-no-plano.md`](specs/SPEC-076-convidados-no-plano.md) · [ADR-060](DECISIONS.md#adr-060) | [#366](https://github.com/RodReis/arenahub/issues/366) | aprovada-pi — decisões de #339 em 18/09/2026 |
| F77 | SPEC-077 | 1 | — | Agenda de aulas: grade recorrente, professor e capacidade | [`SPEC-077-agenda-de-aulas-professor-e-capacidade.md`](specs/SPEC-077-agenda-de-aulas-professor-e-capacidade.md) · [ADR-061](DECISIONS.md#adr-061) | [#367](https://github.com/RodReis/arenahub/issues/367) | aprovada-pi — decisões de #339 em 18/09/2026 |
| F78 | SPEC-078 | 1 | — | Reserva, presença/no-show e aulas inclusas no plano (depende da F77) | [`SPEC-078-reserva-presenca-e-aulas-inclusas-no-plano.md`](specs/SPEC-078-reserva-presenca-e-aulas-inclusas-no-plano.md) · [ADR-061](DECISIONS.md#adr-061) | [#368](https://github.com/RodReis/arenahub/issues/368) | aprovada-pi — decisões de #339 em 18/09/2026 |
| F79 | SPEC-079 | 1 | — | Acesso do Admin do tenant: ver, reenviar, corrigir e revogar | [`SPEC-079-acesso-do-admin-do-tenant.md`](specs/SPEC-079-acesso-do-admin-do-tenant.md) | [#373](https://github.com/RodReis/arenahub/issues/373) | aprovada-pi — decisões do PI em 22/09/2026 |
| F80 | SPEC-080 | 1 | — | Perfis prontos e tela de equipe do tenant (depende da F79) | — | [#374](https://github.com/RodReis/arenahub/issues/374) | aprovada-pi — decisões do PI em 22/09/2026 |
...
| F23 | SPEC-023 | 4 | 4.1 | Identidade e shell mobile | [`SPEC-023-identidade-e-shell-mobile.md`](specs/SPEC-023-identidade-e-shell-mobile.md) | [#23](https://github.com/RodReis/arenahub/issues/23) | ✅ **entregue** em 12/09/2026 — aguardando aceite. **O aluno passou a ser um sujeito autenticável**: até aqui ele era registro que o painel administrava, sem credencial nenhuma. Conta em **tabela própria** (`student_accounts`) e não mais um `User` com papel "aluno" — `User.email` é único **global**, e a mesma pessoa matriculada em duas academias colidiria; o aluno nunca terá papel, permissão, MFA nem elevação de suporte, então toda consulta do painel passaria a filtrar "e não é aluno"; e a conta do aluno morre com a matrícula. Decidido pelo PI. **Sessão reusa o modelo de FAMÍLIA do painel** (rotação, replay revoga tudo), em tabela separada porque `Session.userId` aponta para `User` e não existe join entre sessão de aluno e usuário de painel. **Duas regras puras** com o "agora" por parâmetro, e em ambas a ORDEM DAS CHECAGENS é a regra: no token, estado antes de validade (um token usado responde "usado" hoje e daqui a um ano); na sessão, **replay antes de expiração** — um elo já rotacionado que reaparece é cópia, e continua sendo depois de vencer; invertido, o replay tardio sairia como recusa banal e bastaria ao atacante esperar. **Separação de canais**: o claim `canal` entrou em `ClaimsDeAcesso` e é verificado nos DOIS guards — até aqui o único obstáculo entre um token de aluno e uma rota de painel era o transporte (header vs. cookie), que é acidente, não garantia. **TRÊS DEFEITOS ACHADOS EM REVISÃO ADVERSARIAL**, um deles medindo e não lendo: o hash descartável do login **nunca rodava scrypt** (envelope escrito à mão com 6 campos em vez de 7, e o `conferir` sai antes do scrypt quando o formato não bate) — **20,6 ms com conta contra 0,0 ms sem**, a base inteira enumerável por cronômetro com a mensagem de erro idêntica; a revisão passou por esse item e deu "sem defeito". Mais: o guard montava o contexto a partir do token sem comparar com o tenant da sessão, e o teste HTTP de revogação **não provava step-up** (o login preenche a autenticação com o "agora", então a guarda nunca era exercitada). **DOIS DEFEITOS QUE SÓ A TELA REVELOU**, com todo o gate verde: (1) a Home respondia `UNAVAILABLE` com sessão válida — `students` tem RLS com FORCE, o `TenantRlsInterceptor` conhecia **quatro** origens de identidade e o canal do aluno era a quinta, então a consulta voltava zero linhas **sem erro e sem log**; os 41 testes passavam porque criam o aluno na própria suíte, com contexto já aberto; (2) o login autenticava (API respondia 200) mas **a tela não saía do formulário** — só `app/index.tsx` redirecionava, e os testes cobriam as duas pontas sem exercitar a costura. **TRÊS TESTES QUE PASSAVAM PELO MOTIVO ERRADO**, os três revelados por canário: o de canal (token de painel era recusado por não ter `studentId`, não pelo canal), o de deep link (`startsWith` trocado por `includes` não derrubava nada — o caso caía na validação de caminho) e o de toque duplo (a proteção real está no `Botao`, que se desabilita). **Verificado na tela**, emulador Pixel 10 Pro XL: ativar → recusar token reusado (409) → logar → Home com "Bom dia, ANA" (só o primeiro nome, §5.4) → **fechar e reabrir sem digitar senha** (`M4-AC-001`) → **sessão revogada no banco volta ao login limpo** (`M4-AC-002`). Gate: lint 11/11, typecheck 16/16, build 10/10, teste 14/14 e guardas 6/6; **49 testes no app** e **1068 de integração** (990 na API + 78 no database, medidos no CI — o Jest crasha no Windows depois de passar e o gerador herdava 1028 da entrega anterior). **Com migration** — aditiva (4 enums, 3 tabelas), aplicada em desenvolvimento pelo caminho documentado nas F21/F36/F66/F67/F70, sem drift, e **aplicada em produção** no Pre-Deploy do merge (`ee13ff4`, 12/09/2026 15:02 — `Applying migration 20260912060000_f23_identidade_do_aluno` seguido de `All migrations have been successfully applied`); o deploy seguinte registrou `No pending migrations to apply` com as nove rotas `/api/v1/mobile/*` mapeadas |
| F24 | SPEC-024 | 4 | 4.2 | Carteirinha, plano e frequência | [`SPEC-024-carteirinha-plano-e-frequencia.md`](specs/SPEC-024-carteirinha-plano-e-frequencia.md) | [#24](https://github.com/RodReis/arenahub/issues/24) | ✅ **entregue** em 12/09/2026 — aguardando aceite. **Entregue SEM carteirinha e sem QR, por corte do PI na abertura da fatia**: a Slice 4.2 pede três coisas e as duas de leitura saíram; a carteirinha parou numa pergunta de produto que ninguém tinha respondido — **quem escaneia o QR?** Sem leitor na recepção ele é anti-print visual; com leitor, é endpoint de validação e uma segunda identidade de acesso ao lado da biometria (ADR-008). São desenhos diferentes, e escolher errado custa reescrever. `M4-FR-007` segue **sem executor** até o PI decidir. **Duas rotas de leitura pura** (`/api/v1/mobile/plano` e `/api/v1/mobile/frequencia`), duas telas e nenhuma migration. 🔁 **A frequência NÃO foi reimplementada**: a derivação inteira — passagem elegível, dia civil local, agregação, consistência — já existia no `AttendanceService` desde a **F18**, e o BFF do app só a traduz (`M4-FR-008`: *"frequência derivada pelo backend"*). Foi exportado o **serviço**, nunca o repositório: expor o repositório convidaria o app a montar a própria contagem, e duas contagens divergem — a divergência só apareceria quando alguém comparasse o app com o painel. 🔴 **O defeito que a revisão adversarial achou, e que a suíte inicial não pegava:** o serviço filtrava o entitlement por `status: 'ACTIVE'` **sem a janela de validade**, mas `ACTIVE` só vira `EXPIRED` quando algo o expira — enquanto esse algo não roda, existe linha `ACTIVE` com `endsAt` no passado, e o app diria *"Ativo, válido até 01/08"* num dia 12/09. **O aluno sairia de casa com um direito que a catraca recusa**, porque `access-projection.repository.ts` confere a janela. Corrigido com o mesmo filtro do motor. Junto veio a ordenação: entre dois direitos vigentes — legítimos por **INV-064**, e sem nenhuma única no banco que os impeça — o app mostra o que **termina por último**, que é a resposta para *"até quando posso treinar?"*; ordenar por `startsAt` mostraria a cortesia de uma semana concedida hoje no lugar da mensalidade que vai até dezembro. ⚠️ **E o canário provou que um dos dois testes novos passava pelo motivo errado**: com o aluno tendo **só** o direito vencido, o fallback (*"o mais recente conta a história"*) devolvia a mesma linha e a asserção nunca alcançava a guarda testada — a armadilha de *guarda anterior* pela enésima vez. Refeito com um direito vigente ao lado, os dois caem com o defeito e passam sem ele. ✅ **Nenhum vazamento entre alunos ou tenants**: o `studentId` sai sempre da sessão e os dois endpoints **ignoram** `?studentId=` — há teste que passa o UUID de outro aluno e exige a resposta do dono da sessão. Gate: lint 11/11 e typecheck 16/16 com `--force`, unitário **3276**, integração **1079** (1001 na API + 78 no database, números da execução verde do CI no Linux — o Jest crasha no Windows e o gerador herda número, corrigido à mão; a medição local suíte a suíte deu 1049 e estava **inflada**, porque `--testPathPattern` não ancora o nome e `billing` casa as 8 suítes `billing-*`), guardas **6/6** e self-check **17/17**. Snapshot do OpenAPI regenerado: **208 linhas, só adições**, conferido no diff. **Sem migration** |
| F25 | SPEC-025 | 4 | 4.3 | Financeiro mobile | [`SPEC-025-financeiro-mobile.md`](specs/SPEC-025-financeiro-mobile.md) | [#25](https://github.com/RodReis/arenahub/issues/25) | ✅ **entregue** em 12/09/2026 — aguardando aceite. **Zero desenho novo de billing**: o app reusa os MESMOS casos de uso do MVP 2 que o totem (F52/F53) já consome — `CriarCobrancaPixUseCase`, `CriarCheckoutDeCartaoUseCase`, `ConsultarTentativaUseCase`, mais `EmitirReciboUseCase` (agora também exportado pelo `BillingModule`, só para este BFF). **A diferença para o totem**: aqui o aluno ESCOLHE a invoice pela própria lista, e não o backend sozinho — como `CriarCobrancaPixUseCase`/`ConsultarTentativaUseCase` escopam só por tenant (correto no painel), o `MobileFinanceiroService` confere ANTES de cada chamada que a invoice/tentativa/pagamento pertence ao aluno da sessão (`buscarTentativaDoAluno` já existia; `buscarPagamentoDoAluno` é novo, mesmo padrão, para o recibo). **A idempotência de "fechar/reabrir o app não duplica cobrança" (`M4-AC-005`) vem de graça** dos próprios casos de uso — índice parcial no cartão, reuso de tentativa `PENDING` no PIX — e o teste de integração prova que o BFF não a contorna (duas chamadas seguidas devolvem o MESMO `paymentAttemptId`). **`M4-BR-001`/INV-081** (retorno do checkout nunca confirma pagamento) é a regra central: o polling (`usarStatusDaTentativa`, app) só relê `GET /payment-attempts/:id`, que só lê o que o webhook já escreveu — nunca consulta o provedor, e para sozinho ao atingir status terminal ou ao desmontar. Endpoints novos: `GET /invoices`, `POST /invoices/:id/pix`, `POST /invoices/:id/checkout`, `GET /payment-attempts/:id`, `POST /payments/:id/receipt`. Telas novas: lista de faturas (`financeiro.tsx`), escolha de método + QR + acompanhamento (`pagamento.tsx`). Gate: lint 11/11, typecheck 16/16 e build 10/10 com `--force`; unitário **3300** (10 testes do `MobileFinanceiroService` + 9 de tela/hook do app); integração **1088** (1079 + 9 da suíte nova `mobile-financeiro.int-spec.ts`, corrigido à mão — o Jest crasha no Windows depois de passar e o gerador herda o número anterior). Snapshot do OpenAPI regenerado: **311 linhas, só adições**, conferido no diff. **Sem migration** |
| F26 | SPEC-026 | 4 | 4.4 | Avaliações e consentimentos | [`SPEC-026-avaliacoes-e-consentimentos.md`](specs/SPEC-026-avaliacoes-e-consentimentos.md) | [#26](https://github.com/RodReis/arenahub/issues/26) | ✅ **entregue** em 12/09/2026, **aceita pelo PI em 13/09/2026**. Reusa `HealthProgressService`/`AiAnalysisService` (zero regra nova de avaliação); generaliza `ConsentRepository` (era só `'BIOMETRIC'`) para gestão self-service de consentimento no app; reforma `HealthExportService` de síncrono para assíncrono (`ADR-056`, painel e app pelo mesmo caminho). `ConsentDocumentType` ganha `TERMS`/`PRIVACY`/`MARKETING`. `BIOMETRIC` fica fora do self-service (cadastro presencial, revogação exige exclusão física nos leitores — fatia futura). Ver `docs/DEVELOPMENT.md` e `ADR-056` |
| ~~F27~~ | ~~SPEC-027~~ | 4 | 4.5 | ~~Kiosk seguro~~ | **executada pela F49** — [ADR-042](DECISIONS.md#adr-042), que antecipou as Slices 4.5 e 4.6 para o MVP 3.5 | [#27](https://github.com/RodReis/arenahub/issues/27) | 🔒 **número queimado**. Mesmo título da F49 (*Kiosk seguro*) e o **mesmo aceite palavra por palavra** (*"bateria automatizada e manual comprova que dado do aluno A não aparece para o aluno B"*), entregue em **25/08/2026**. Cinco dos seis itens da Slice 4.5 estão na F49; o sexto — *plano, invoices e PIX* — foi para a **F52**, no mesmo MVP 3.5. **As duas pendências de UI do card não sobraram:** o timeout virou configurável (sessão de 60 s, `duracaoSegundos`), e *o que o totem imprime* nunca foi requisito — `M4-FR-020` manda **limpar** a fila de impressão, não imprimir. Card #27 aguarda só o **fechamento pelo PI** |
| ~~F28~~ | ~~SPEC-028~~ | 4 | 4.6 | ~~Pagamento e desbloqueio no totem~~ | **executada pela F52** — [ADR-042](DECISIONS.md#adr-042), que antecipou as Slices 4.5 e 4.6 para o MVP 3.5 | [#28](https://github.com/RodReis/arenahub/issues/28) | 🔒 **número queimado**. A Slice 4.6 é a mesma nos dois cards, com o **mesmo aceite palavra por palavra** (*"aluno paga e tem entitlement restaurado pelo fluxo do MVP 2, sem bypass local"*), e foi entregue pela **F52 em 26/08/2026** ([#209](https://github.com/RodReis/arenahub/pull/209)). **Sem ponta aberta:** a emenda do [ADR-043](DECISIONS.md#adr-043), Decisão 4 — dois QRs, PIX **e** cartão — entrou no mesmo PR, não ficou para depois. Card #28 aguarda só o **fechamento pelo PI** |
| F29 | SPEC-029 | 4 | 4.7 | Piloto e distribuição | [`SPEC-029-piloto-e-distribuicao.md`](specs/SPEC-029-piloto-e-distribuicao.md) | [#29](https://github.com/RodReis/arenahub/issues/29) | 🟡 **parcial** em 14/09/2026 — o que é código entrou; loja, piloto e adapter de push ficaram, por decisão do PI na abertura da fatia. **O PI adiou `M4-DIST-01`** (publicação em loja): distribuição interna cobre o piloto, e prazo de revisão de loja é externo. **Escolheu OneSignal** como provedor de push, cujas credenciais ainda não existem — então a porta `PushProvider` entrou com dublê e `PUSH_NOTIFICATIONS` nasce desligado. **Entregue:** (1) caixa de avisos interna (`GET /mobile/avisos`, `POST /:id/lido`, tela no app) — *a caixa é a entrega e o push é só um atalho até ela*, então quem recusa notificação vê os mesmos avisos; (2) telemetria por **allowlist** (`POST /mobile/telemetria`), oposta ao denylist do logger do edge-agent, porque o emissor é um app que não controlamos e campo não previsto vazaria para o provedor de crash; (3) **versão mínima e kill switches** — `versionPolicy` existia na Home desde a F23 mas era fixo em `SUPPORTED`, e trocá-lo pela regra real **não derrubou teste nenhum**, o que revelou que ninguém cobria o campo. **Não entregue, e por quê:** builds assinados e publicação (dependem de conta de loja e certificado), piloto com coortes (decisão de produto e assinatura de cinco áreas), adapter OneSignal (credenciais). Gate: lint 11/11, typecheck 16/16, build 10/10 `--force`, unitário 3415, integração **76/76 suítes e 1044 testes confirmados pelo CI**. 🔴 **O CI achou dois problemas que a bancada local não podia achar:** heap V8 estourando (OOM, exit 134) na 76ª suíte, por acúmulo entre suítes já documentado desde a F50 — corrigido subindo `--max-old-space-size` para 10240 (issue #327 registra o conserto de raiz); e um teste de frequência da F24 (não desta fatia) quebrando de verdade quando o CI roda perto da virada de meia-noite em São Paulo, porque os treinos de fixture usavam `Date.now()` relativo em vez de horário fixo — corrigido ancorando no meio-dia UTC, 15h de margem de qualquer virada (issue #328) |
...
| F43 | SPEC-043 | 2.5 | 2.5.2 | Design system da superfície `mobile` | [`SPEC-043-design-system-do-app.md`](specs/SPEC-043-design-system-do-app.md) | [#82](https://github.com/RodReis/arenahub/issues/82) | ✅ **entregue** em 12/09/2026 ([#321](https://github.com/RodReis/arenahub/pull/321)) — aguardando aceite. **O gate abriu**: o PI mandou fazer F43 e F23 em 11/09, e escolheu a ordem — design system primeiro, telas depois. **Terceira superfície no pipeline de tokens**, com `app.json` e o mesmo gate de contraste do painel e do totem (48 pares, contra 13 antes). O app não emite CSS — React Native não lê custom property — então a saída é só TypeScript, em entrypoint próprio (`@arenahub/ui/app-tokens`) para não arrastar `react-dom` e `recharts` para o bundle do celular. **Light mode validado agora** (SPEC-043 §5 pergunta 2, decidida pelo PI): reusa a paleta do painel em vez de derivar outra, porque o dark do app **já é** a rampa carbon invertida — derivar criaria hex que nenhum documento define. **O gate achou três defeitos escritos no próprio DS ou nos meus números**: (1) `border/default` do dark entregava **1,19** sobre `bg/raised` contra o alvo de 3,0 da WCAG 1.4.11 — a borda que delimita o alvo tocável do campo e do botão secundário, num app usado sob luz de academia; mesmo defeito e mesma correção do totem (`#232A3D` → `#51618C`, PR #232), vai para `#646D79`; (2) o tint do badge **não podia ser número único** — 16% é a receita do dark, e sobre branco o mesmo valor entrega 4,43 no `err` e reprova, então o light usa os 10% do painel; (3) `accent/soft` **troca de papel** entre os temas (tinta no dark, fundo tonal no light) e medi-lo como texto no light reprovava a pergunta errada. Seis `contrastOnApp` escritos à mão divergiam do medido em até 1,4 ponto. **Os quatro padrões que o app não pode errar** entregues com teste que cai se o padrão cair: espera que nunca escreve "Pago", carteirinha com token opaco sem PII, aviso de IA persistente (`dismissible: false` no tipo) e engajamento que nasce desligado. `state-labels.ts` é **compartilhado** com o painel pelo entrypoint novo `@arenahub/ui/domain`, não duplicado (SPEC-043 §6). **Provado por canário duas vezes**: 4 no gate de tokens (borda revertida, número mentiroso, alvo de toque abaixo de 44 px, tint do light em 16%) e 4 nos padrões críticos. **Verificado na tela, e a tela achou o que o CI não pegava**: aberto no emulador Pixel 10 Pro XL (Android 17) nos dois temas, o claro revelou que **nenhum card desenhava borda** — o `comBorda` nascia `false` e ninguém passava, deixando inerte o token recém-corrigido; somado a isso a separação entre as três superfícies fica entre 1,07 e 1,22 **nos dois temas**, e nesse patamar não separa nada. A prop virou `semBorda` com o padrão invertido. **Depois disso o DS-APP subiu para v2.1 na `main`** — rebrand azul royal alinhado ao totem, botão primário com **gradiente** e tema claro completo com os neutros do painel — e a fatia foi **migrada**: os componentes não mudaram (leem token), mudou a camada de cor. Duas decisões de gate na migração, ambas com precedente: **dois tokens de borda** onde a v2.1 traz um (o hex do documento no escuro é *literalmente* o que o totem reprovou no PR #232), e **dois pares reportados sem quebrar o build**, como o totem já faz com o CTA. E **uma guarda nova que contraste não dá**: um canário mostrou que a razão é **simétrica**, então QR invertido mede igual ao certo e passaria batido — a guarda agora afirma a **luminância absoluta** de cada lado. **`apps/mobile/app/index.tsx` é vitrine, não produto** — existe pelo risco que o ADR-025 registrou (componente sem consumidor erra em silêncio), e foi ela que revelou a borda faltando; as telas reais são da F23 em diante. Gate: lint 11/11, typecheck 16/16, build 10/10 com bundles iOS e Android exportados, teste 14/14, guardas 6/6. **Sem migration** |
...
| F57 | SPEC-057 | 1 | — | Dashboard operacional (nova porta de entrada do painel) | [`SPEC-057-dashboard-operacional.md`](specs/SPEC-057-dashboard-operacional.md) | [#242](https://github.com/RodReis/arenahub/issues/242) | ✅ **entregue** em 01/09/2026 ([#247](https://github.com/RodReis/arenahub/pull/247)) — aguardando aceite. Os sete blocos num aceite só, com feriados dentro (a proposta de uma **fatia própria de feriados** foi recusada pelo PI em 01/09; o número F58 ficou livre e foi **alocado à implantação em 02/09/2026** — ver a linha seguinte). Dois pedidos do PI entraram junto: **seletor de unidade** no topo (a troca estava adiada desde a F45, e sem ela não existe "hoje") e um **passe visual mais expressivo**, que emendou o `PRODUCT.md` e o `DS-PAINEL.md` §2.9/§2.10 |
| F58 | SPEC-058 | 1 | — | Implantação: API e painel na Railway, totem local, pré-produção | [`SPEC-058-implantacao-nuvem-e-totem-local.md`](specs/SPEC-058-implantacao-nuvem-e-totem-local.md) · [ADR-051](DECISIONS.md#adr-051) | [#253](https://github.com/RodReis/arenahub/issues/253) | 🚧 **em andamento** — código mesclado ([#255](https://github.com/RodReis/arenahub/pull/255), com [#256](https://github.com/RodReis/arenahub/pull/256), [#257](https://github.com/RodReis/arenahub/pull/257) e [#258](https://github.com/RodReis/arenahub/pull/258) corrigindo o build de deploy): `REDIS_URL` obrigatória em produção, rate limit no login (10 tentativas erradas/min por IP+e-mail), porta 3210 fixa no `start` do kiosk, bootstrap de tenant idempotente. **Nuvem no ar em 02/09/2026:** Postgres, Redis e Bucket provisionados; **API respondendo** (`Nest application successfully started`, 55 migrations conferidas pelo pre-deploy), **painel aberto pelo PI** e **tenant real criado** (`arena-positiva`, unidade `MATRIZ`, `OWNER` com e-mail real; nenhum usuário `.test` em produção — AC-4). O bootstrap concedia 7 das 42 permissões e o dono levava `FORBIDDEN` no próprio dashboard: corrigido em [#259](https://github.com/RodReis/arenahub/pull/259), que tirou a lista das duas cópias e criou fonte única. Falta o backup do Postgres (AC-8) e o §5.3–§8 do runbook — base do Pacto, totem físico e as evidências. **Não é produção**: restrição 1 do ADR-029 segue em vigor. **02/09/2026 — o import da base do Pacto (§5.3) ganhou três correções antes de rodar em pré-produção** ([#260](https://github.com/RodReis/arenahub/issues/260) · [#261](https://github.com/RodReis/arenahub/pull/261)): a UF era fixa em `GO` e ignorava a coluna do arquivo; o endereço exigia CEP junto do logradouro e descartava 275 ruas por causa de 69 CEPs; e endereço nunca era atualizado, só criado. O CLI passou a ler CSV além de JSON. Decisão do PI na mesma conversa: sem `Data Início` no arquivo, vale **01/09/2026** com fim em 12 meses — sem período não há `Entitlement`, e a catraca ficaria fechada para a base inteira. **A base do Pacto (§5.3) FOI IMPORTADA EM PRODUÇÃO em 02/09/2026** ([#262](https://github.com/RodReis/arenahub/pull/262)): as 661 credenciais do leitor real (333 cartões + 328 faciais, 330 alunos) **já estavam lá**, mas com **0 assinaturas e 0 direitos** — ninguém passava na catraca mesmo com o cartão gravado no equipamento. O import fechou o elo: **297 assinaturas, 330 direitos** (297 por assinatura, 24 funcionários, 9 personal trainers), 2310 janelas de unidade, e **0 alunos com cartão sem acesso**. O nome do plano virou `ARENAHUB_PLANO` porque produção usa `Plano Individuais - protocolos e acompanhamento`, e não o nome da bancada. **Risco registrado:** rodar pelo proxy TCP público flerta com o timeout de 5s de transação interativa do Prisma — uma pessoa caiu em `erro ao gravar` a 5527 ms e foi recuperada rerodando o seed com um CSV de uma linha (a idempotência funcionando); de dentro da rede da Railway o problema não existe. Rodado **sem backup**, por decisão do PI — o AC-8 segue pendente |
...
| F60 | SPEC-060 | 1 | — | Modalidades por unidade e vínculo do aluno | [`SPEC-060-modalidades-por-unidade.md`](specs/SPEC-060-modalidades-por-unidade.md) | [#270](https://github.com/RodReis/arenahub/issues/270) | ✅ **entregue** em 04/09/2026 — aguardando aceite. **Escopo decidido pelo PI na mesma conversa:** a Arena Positiva é um complexo esportivo (quadras de areia, cross fit, box), e o ArenaHub só conhecia "aluno da academia". Modalidade é **rótulo, não controle de acesso** — não toca em plano nem no motor de decisão, e hoje só a academia tem catraca. **Por unidade**, não por tenant; **várias por aluno**; **opcional na coluna, obrigatória no wizard**. A migração atribuiu `Academia - Clínica de Musculação` aos **1912 alunos com `profile = STUDENT`** (dos 1984 totais — funcionário, personal e administrador ficam de fora), idempotente nos dois `INSERT` e rodada em local; **falta rodar em produção**. A lista de alunos ganhou o **combo de modalidade** no filtro, com a lista de opções acompanhando a unidade escolhida (decisão do PI): a combinação *unidade A + modalidade da unidade B* não é oferecida, porque só devolveria lista vazia. O total do rodapé usa o mesmo `where` da listagem, por função compartilhada
| F61 | SPEC-061 | 7 | — | Super Admin e ciclo de vida do tenant (substitui o bootstrap por script) | [`SPEC-061-super-admin-e-ciclo-de-vida-do-tenant.md`](specs/SPEC-061-super-admin-e-ciclo-de-vida-do-tenant.md) · [ADR-052](DECISIONS.md#adr-052) | [#284](https://github.com/RodReis/arenahub/issues/284) | ✅ **entregue** em 09/09/2026 — aguardando aceite. **Cinco decisões que o ADR-052 não fechava, decididas pelo PI:** `PlatformAuditLog` separado (o `AuditLog` exige `tenant_id` com FK, e "criou o tenant X" não tem tenant dono); `INACTIVE` = desligado pelo dono, `SUSPENDED` = inadimplência (quem escreve é a F65); MFA obrigatório **só** para o Super Admin, ligando o pre-auth que existia morto desde a fatia que o criou; tabela `PlatformAdmin` **fora** de `roles`/`permissions` (afrouxar `@@unique([tenantId, name])` poria papel global e de tenant na mesma query de autorização); e `Session.tenantId` nullable, com a elevação em registro próprio. **Três defeitos achados na revisão, cada um com canário provado nos dois estados:** o convite do OWNER carregava o `gymUnitId` da matriz — e esse campo vira o `UserRole` ao aceitar, então o dono não enxergaria a segunda unidade no dia em que ela abrisse; elevar para um segundo tenant empilhava duas `SupportElevation` vivas na mesma sessão, e como o encerrar fecha só a mais recente, uma saída deixaria a primeira aberta e órfã (o bypass silencioso que INV-005 proíbe); e o login do painel tratava o desafio de MFA (HTTP 200, sem cookie) como sucesso, redirecionando para um laço com a senha certa. **Lacuna fechada em 09/09/2026** pela [#293](https://github.com/RodReis/arenahub/issues/293): o painel ganhou as duas telas de segundo fator, e o Super Admin entra pelo formulário. **O `bootstrap-tenant.ts` fica:** o CRUD substitui o script para *criar tenant*, mas o primeiro Super Admin ainda nasce fora do painel |
| F62 | SPEC-062 | 7 | — | Identidade visual do tenant (logo, ícone, nome, missão, diferenciais) e login por slug | [`SPEC-062-identidade-visual-do-tenant-e-login-por-slug.md`](specs/SPEC-062-identidade-visual-do-tenant-e-login-por-slug.md) · [ADR-052](DECISIONS.md#adr-052) | [#285](https://github.com/RodReis/arenahub/issues/285) | ✅ **entregue** em 09/09/2026 — aguardando aceite. **A tela de login passou a dizer de quem ela e.** `/{slug}/login` carrega logo, nome, missao e diferenciais da academia; `/login` sem slug segue com a marca ArenaHub. **SVG e o unico upload do produto que aceita vetor, e por isso o unico com sanitizacao:** `<script>`, handler `on*`, `<foreignObject>`, `javascript:` em atributo e `<use href="http...">` sao **recusados no upload, nao removidos** — reescrever o arquivo de alguem entrega logo diferente do que a pessoa escolheu, e sanitizador que "limpa" falha calado. O antivirus **nao substitui** isso: script dentro de um vetor nao casa com assinatura de virus nenhuma. Os cabecalhos da resposta (`CSP: default-src 'none'; sandbox`, `nosniff`) sao a segunda trava, para o arquivo que entrar por um caminho nao previsto. **O caminho de fuga obvio fecha na assinatura dos bytes:** declarar `image/png` e enviar o SVG com script pularia a sanitizacao inteira, e e recusado por `FILE_SIGNATURE_MISMATCH`. **A rota publica nao pode dizer que um slug nao existe** (aceite da issue): slug desconhecido e tenant fora de operacao devolvem a **mesma** resposta — mesmo status e mesmo corpo —, senao a tela de entrada viraria oraculo de quais academias sao clientes do ArenaHub, e quais estao suspensas. O teste compara as duas respostas com `toEqual`, e nao so o status. **A chave do objeto e conferida contra o prefixo do tenant antes de servir:** ela sai de uma coluna do banco, e servir o que a coluna disser entregaria a foto biometrica de um aluno — que mora no mesmo bucket — por rota publica. **O painel serve a imagem por route handler proprio** (`/marca/{slug}/{peca}`), e nao pelo endereco da API: `API_INTERNAL_URL` e o endereco visto de dentro da rede, e publica-lo no bundle e o que o `next.config.ts` proibe; um `NEXT_PUBLIC_API_URL` criaria uma segunda configuracao que a primeira mudanca de ambiente faz divergir. **A marca da academia SUBSTITUI o discurso do produto, nao se soma a ele** — e academia que nao escreveu missao nem diferenciais volta ao texto do ArenaHub, porque uma coluna com um nome e nada mais seria pior que a tela anterior a fatia. Provado por canario nos dois estados: desligar a sanitizacao derruba 9 unitarios e o E2E na linha da recusa; empilhar marca e discurso derruba o teste que existe so para isso. Gate: lint 10/10 e typecheck 15/15 com `--force`, build 9/9, unitario 13/13 tarefas (575 no painel, 24 novos no dominio de identidade), **integracao 63 suites / 859** (numero do CI, que roda em Linux sem o crash do Windows) — o CI achou o que a bancada local nao podia achar: a suite falava com MinIO de verdade e o runner so sobe Postgres, entao ela passou a usar dublê que guarda por chave e **lanca em chave ausente, como o S3**; o canario continua derrubando a suite com a sanitizacao desligada, **E2E 2/2** no `platform.e2e-spec.ts`. **Com migration** — falta rodar em producao |
| F63 | SPEC-063 | 7 | — | Plano SaaS (por aluno ativo/inativo ou fixo corrigido por índice) e contrato imutável | [`SPEC-063-plano-saas-e-contrato.md`](specs/SPEC-063-plano-saas-e-contrato.md) · [ADR-052](DECISIONS.md#adr-052) | [#286](https://github.com/RodReis/arenahub/issues/286) | ✅ **entregue** em 09/09/2026 — aguardando aceite. **O ArenaHub passou a saber quanto cobra de cada academia.** Plano SaaS em dois modelos, contrato imutável com os valores acordados, PDF no fechamento e histórico manual do índice. **Os valores do contrato são cópia, não referência** — nada no caso de uso volta ao `SaasPlan` para saber quanto cobrar, e o gerador de PDF sequer aceita o plano como parâmetro: é a única forma de garantir que alterar o preço do catálogo não mexa em contrato já fechado. O canário revelou que a primeira versão do teste passava pelo motivo errado (alterava o plano *depois* de ativar, com o PDF já gravado); corrigido para a janela entre o rascunho e a ativação, ele derruba o teste. **O índice acumula, não soma** — 1% e depois 1% dá 2,01% —, é inteiro em milésimos de ponto percentual e arredonda uma vez só, no fim. **Sem valor cadastrado a correção não roda**, e o erro diz quais competências faltam: aplicar zero inventaria que o índice foi zero, e pular o mês mudaria a janela — as duas produzem fatura errada que parece certa. **Cinco guardas no banco**, entre elas o índice parcial que garante um contrato vigente por academia (`UNIQUE(tenant, status)` aceitaria dois rascunhos e recusaria o segundo encerrado) e o CHECK que recusa contrato ativo sem PDF. **O painel serve o PDF por route handler próprio**, que repassa o cookie de acesso e por isso jamais pode virar proxy genérico. Gate: lint 10/10 e typecheck 15/15 com `--force`, build 9/9, unitário 13/13 tarefas, **integração 877 + 78** (18 novos). O fechamento fica fora do E2E porque o runner do CI não sobe MinIO. **Com migration** — falta rodar em produção |
| F64 | SPEC-064 | 7 | — | Fatura da plataforma sobre o tenant (inativo = todo status ≠ ACTIVE; emissão dia 1, configurável) | [`SPEC-064-fatura-da-plataforma.md`](specs/SPEC-064-fatura-da-plataforma.md) · [ADR-052](DECISIONS.md#adr-052) | [#287](https://github.com/RodReis/arenahub/issues/287) | ✅ **entregue** em 09/09/2026 — aguardando aceite. **O ArenaHub passou a cobrar a academia.** No dia de emissão do contrato conta os alunos por status, emite a fatura e o Super Admin registra o pagamento; o tenant vê a prévia antes. **A contagem é congelada** — `emitir` conta uma vez e grava, e nada recalcula depois: a fatura gravada não tem caminho de volta até a tabela de alunos, e a base toda cancelando no dia 10 não mexe na fatura do dia 1. **A idempotência mora no banco**: a chave única `(tenant_id, competence)` recusa a segunda emissão, e `create` dentro de `try` — não `upsert` — é o que impede a reexecução de atualizar a fatura com a contagem de hoje. Duas emissões simultâneas são disparadas em paralelo no teste. **Inativo é contado por complemento**, não enumerando os seis status: um status novo cai nele sozinho, enquanto enumerar o deixaria fora das duas contagens e a fatura sairia a menos sem nada falhar. **Tudo em UTC** — em São Paulo o dia 1 local é o dia 30 do mês anterior em UTC, e o job de madrugada faturaria o mês errado com a chave única aceitando as duas linhas. **Cinco guardas no banco**, entre elas competência sempre no primeiro dia do mês e `paid_at` andando junto com o status. **A prévia decompõe a conta**: o risco que o PI aceitou é a fatura ser dominada por quem não treina — 66% na base real, R$ 5.962,50 sendo 401 ativos × R$ 5,00 e 1.583 inativos × R$ 2,50 —, e um total sozinho esconderia isso. A lista de academias ganhou colunas de ativos e inativos (pedido do PI). **Lacuna pré-existente registrada:** a lista de rotas em prosa do teste de OpenAPI usa `arrayContaining` e só pega rota que some, nunca rota não declarada. Gate: lint 10/10 e typecheck 15/15 com `--force`, build 9/9, unitário 13/13 tarefas, **integração 889 + 78**, **E2E 80 + 2**. **Verificado na tela** com a base real. **Com migration** — falta rodar em produção |
| F65 | SPEC-065 | 7 | — | Gate de tenant no motor de decisão — carência 15 dias, razão `TENANT_SUSPENDED` | [`SPEC-065-gate-de-tenant-no-motor-de-decisao.md`](specs/SPEC-065-gate-de-tenant-no-motor-de-decisao.md) · [ADR-053](DECISIONS.md#adr-053) | [#288](https://github.com/RodReis/arenahub/issues/288) | ✅ **entregue** em 09/09/2026 pelo PR [#300](https://github.com/RodReis/arenahub/pull/300) — **aceita pelo PI** |
| F66 | SPEC-066 | 7 | — | RLS fase 1 — role de runtime, `SET LOCAL` por transação, `students` e `audit_logs` | [`SPEC-066-rls-fase-1-role-de-runtime-e-primeiras-politicas.md`](specs/SPEC-066-rls-fase-1-role-de-runtime-e-primeiras-politicas.md) · [ADR-054](DECISIONS.md#adr-054) | [#289](https://github.com/RodReis/arenahub/issues/289) | ✅ **entregue** em 10/09/2026 — aguardando aceite. **O Postgres passou a recusar sozinho o que a aplicação deixar passar** em `students` e `audit_logs`. RLS é camada 2: nenhum repositório perdeu o `where`, e nenhum dos 81 pontos que injetam o `PrismaService` mudou. **O padrão publicado na documentação do Prisma para RLS não serve a este repositório**, e foi preciso provar isso contra Postgres 17 antes de escolher o desenho: ele envolve cada query num `$transaction([set_config, query])` próprio, que roda em **outra conexão** quando já existe transação aberta — e como há mais de trinta transações interativas aqui, a escrita seria recusada com `42501` em toda uma classe de casos de uso. O que funciona é emitir `set_config` **dentro da própria transação**, e é o que o `PrismaService` faz. **Interceptor, e não middleware:** no Nest o middleware roda antes dos guards, e quem põe o `TenantContext` na requisição é o `AuthGuard` — o escopo nasceria vazio em toda requisição autenticada. **Três achados no caminho:** superusuário ignora RLS mesmo com `FORCE`, então a proteção real vem de a aplicação usar `RUNTIME_DATABASE_URL` e o role de migração de produção não poder ser superusuário; o role restrito precisa de acesso a **todas** as tabelas de negócio, não só às duas com política (conceder menos derrubou a integração inteira com `permission denied for table tenants`); e a elevação de suporte vencida não pode virar contexto `platform`, ou a sessão seguiria lendo todos os tenants depois de expirar. **Sete casos de integração sob o role restrito**, provados por canário — o canário achou um defeito no próprio teste, cuja escrita cruzada falhava por chave estrangeira e passaria com a política desligada. A suíte **pula** sem `RUNTIME_INTEGRATION_DATABASE_URL`, em vez de cair no role dono e passar em falso. **O E2E precisou desligar o role restrito nas duas superfícies**, ou a API leria o banco de desenvolvimento com a configuração parecendo correta. Gate: lint 10/10 e typecheck 15/15 com `--force`, build 9/9, unitário 3126, integração 918 + 78 numa rodada completa sem `FAIL`, E2E 80 + 2. **Com migration** — falta rodar em produção |
| F67 | SPEC-067 | 7 | — | RLS fase 2 — `tenant_id` nas tabelas restantes | [`SPEC-067-rls-fase-2-tenant-id-nas-tabelas-restantes.md`](specs/SPEC-067-rls-fase-2-tenant-id-nas-tabelas-restantes.md) · [ADR-054](DECISIONS.md#adr-054) | [#290](https://github.com/RodReis/arenahub/issues/290) | ✅ **entregue** em 10/09/2026 — aguardando aceite. **Seis tabelas deixaram de depender de `JOIN` para saber de quem é a linha**: `access_passages`, `plan_units`, `plan_access_windows`, `entitlement_unit_windows`, `ranking_entries` e `replay_nonces` ganharam `tenant_id` com FK e índice, por migration de três etapas (coluna nula → backfill pelo pai → `NOT NULL`). **5672 linhas reais migradas com zero divergência** contra o tenant do pai, conferido no banco de desenvolvimento. **A fatia entregou menos do que o ADR-054 §5 pede, e por motivo medido.** A política nas ~100 tabelas com `tenant_id` foi para a **F68**, porque ligá-la quebra a aplicação de duas formas que só aparecem na tela: (1) **seis tabelas de autenticação** — `sessions`, `tenant_memberships`, `roles`, `user_roles`, `invitations`, `support_elevations` — são o caminho que **descobre** o tenant, e com política o login recusa credencial correta (sem contexto o banco devolve 0 vínculos; com contexto, 1); (2) as **394 queries fora de transação** não recebem contexto, porque o `set_config` só entra no `$transaction` interativo da F66 — a tela de planos carregou vazia com 6 planos no banco. **Nem os testes de integração nem os 80 de E2E pegaram** os dois: os primeiros exercitam repositório com contexto já aberto, e o E2E rodava com o role restrito **desligado**. **Três emendas ao ADR-054 §5, decididas pelo PI em 10/09:** `AiPromptVersion` (prompt é do produto, `name` único **globalmente**), `KioskReplayNonce` (anti-replay por `key_id`, sem laço com tenant) e `InboxReceipt` (sem pai de onde herdar e **sem escritor em todo o repositório**) não ganham a coluna; `IndexValue`, `PlatformAdmin` e `SaasPlan` entram na lista de globais. **Ligar o role restrito no E2E foi tentado e revertido:** ele expõe um defeito **pré-existente da F66** — 22 dos 80 testes falham com `42501` ao escrever em `audit_logs`, porque o Super Admin não tem tenant e o contexto não vira `platform` ao criar academia ou elevar suporte. Card [#302](https://github.com/RodReis/arenahub/issues/302); a mesma raiz da F68. **A guarda que sobrevive à fatia** lê o catálogo do Postgres e recusa tabela nova sem `tenant_id` — provada por canário: uma tabela criada à mão derrubou o teste pelo nome. Gate: lint 10/10 e typecheck 15/15 com `--force`, build 9/9, unitário 13/13 tarefas, **integração 926 + 78**, E2E 80 + 2. **Verificado na tela** com a base real. **Com migration** — falta rodar em produção |

| F69 | SPEC-069 | 7 | — | Superfícies contratadas: mobile e totem como flag negociável do plano e do contrato | [`SPEC-069-superficies-contratadas.md`](specs/SPEC-069-superficies-contratadas.md) · [ADR-052](DECISIONS.md#adr-052) | [#317](https://github.com/RodReis/arenahub/issues/317) | ✅ **entregue** em 11/09/2026 — aguardando aceite. **O plano SaaS deixou de vender pacote fechado.** `mobileEnabled` e `kioskEnabled` no plano **e** no contrato: o catálogo traz o padrão, o contrato copia e **pode divergir** — a negociação é por cliente (decisão do PI). Quem recusa lê o **contrato**, nunca o plano, pela mesma regra dos preços. **`DEFAULT true`** porque contrato já fechado nunca autorizou perder acesso — nascer `false` pararia a catraca de toda academia existente no dia do deploy. **Academia sem contrato ativo passa:** o totem é o primeiro equipamento testado na implantação, e recusar viraria "contrato não assinado" = "catraca não funciona" no dia da instalação; inadimplência continua com o gate da F65. A recusa fica **depois da assinatura e antes do nonce** — depois para não dar consulta de graça a chave inventada, antes porque gravar o nonce é o único efeito colateral e recusar depois faria o reenvio pós-renegociação cair em `EDGE_REPLAY_DETECTED`, mandando investigar ataque onde houve acordo comercial. Código próprio **`CONTRACT_KIOSK_DISABLED`**, e não `EDGE_KEY_REVOKED`: emitir credencial nova não resolve superfície fora do contrato. **O PDF imprime as superfícies não contratadas**, porque o documento calado é o que gera a discussão depois. A flag do **mobile fica gravada sem executor** até o MVP 4 — `apps/mobile/` ainda é um `.gitkeep`. Canário nas três guardas: PDF com flag trocada, Server Action sem os campos no corpo, guarda do totem desligada (derruba 1 de 13, os outros 12 seguem verdes). Gate: lint 10/10 e typecheck 15/15 com `--force`, unitário 13/13 (2013), **integração 36 suítes sem `FAIL`**, **E2E 80 + 2**. **Com migration** — **aplicada em produção** em 11/09/2026 pelo Pre-Deploy da Railway (`prisma migrate deploy`), confirmada no log do deploy `d9139999` |
| F72 | SPEC-072 | 1 | — | Foto do aluno na ficha do painel | — | [#348](https://github.com/RodReis/arenahub/issues/348) | ✅ **entregue** em 15/09/2026 — aguardando aceite. Ver resumo completo em `DEVELOPMENT.md`. `Student.photoObjectKey` (chave privada, nunca URL), `PUT`/`GET /api/v1/students/:id/photo` sob permissões já existentes, SVG recusado (foto de pessoa não é vetor), teto de 5 MB. **Com migration** |
| F74 | SPEC-074 | 1 | — | KPIs executivos no painel financeiro (§64/§117): alunos ativos, novos, cancelamentos, churn e LTV | [`SPEC-074-kpis-executivos-painel-financeiro.md`](specs/SPEC-074-kpis-executivos-painel-financeiro.md) | [#357](https://github.com/RodReis/arenahub/issues/357) | ✅ **entregue** em 17/09/2026 ([#358](https://github.com/RodReis/arenahub/pull/358)) — aguardando aceite. Ver resumo completo em `DEVELOPMENT.md` |
| F74 | SPEC-074 | 1 | — | KPIs executivos no painel financeiro (§64/§117): alunos ativos, novos, cancelamentos, churn e LTV | [`SPEC-074-kpis-executivos-painel-financeiro.md`](specs/SPEC-074-kpis-executivos-painel-financeiro.md) | [#357](https://github.com/RodReis/arenahub/issues/357) | aprovada-pi — decisões de #340 em 17/09/2026 |
| F71 | SPEC-071 | 4 | — | Primeiro acesso self-service por CPF e data de nascimento | [`SPEC-071-primeiro-acesso-self-service.md`](specs/SPEC-071-primeiro-acesso-self-service.md) · [ADR-057](DECISIONS.md#adr-057) | [#333](https://github.com/RodReis/arenahub/issues/333) | ✅ **entregue** em 15/09/2026 — aguardando aceite. **Endpoints NOVOS, prefixo próprio** (`POST /api/v1/mobile/activation/lookup` e `.../self-service`, `StudentActivationController`) — SPEC-071 §7 os separa do `/auth` existente de propósito: a consulta (CPF + nascimento) devolve nome completo, CPF, nascimento, plano, local e data de início formatados (§6.3) mais um `activationRef` opaco de curta duração, sem abrir sessão; a confirmação (senha + confirmação, piso de 10 caracteres) troca o `activationRef` pela sessão aberta. **`activationRef` é um token pré-auth assinado** (`TokenService.emitirPreAuth`, o mesmo mecanismo do desafio de MFA, novo `purpose: STUDENT_SELF_SERVICE_ACTIVATION`) — evita migração e reaproveita a disciplina de expiração já testada; a exclusão mútua de verdade mora em `criarOuAtivarConta` (banco), não no token. **Busca por `Student.cpfHash`**, não por `StudentAccount.identifier`: o aluno pode nunca ter recebido convite. Rota é `@Public()`, sem `TenantRlsInterceptor` — o repositório abre o próprio escopo de RLS com `comContexto`, pelo tenant já resolvido do `tenantSlug`; sem isso a consulta veria zero linhas em silêncio sob o role restrito. **Login mostra só CPF na UI, mas o backend aceita `cpf` OU `identificador` (e-mail/telefone) por baixo** (ADR-057, Decisão 6) — quem já ativou pela F23 antes desta fatia não fica sem entrar; recuperação de senha **não muda**, segue token-por-e-mail (SPEC-071 não a toca). **`STUDENT_SELF_SERVICE_ACTIVATION` coexiste com o convite da F23** em corrida (Decisão 5): `criarOuAtivarConta` ativa uma conta `PENDING` por cima, ou cria uma nova quando não havia conta nenhuma — o primeiro a definir a senha vence, e a segunda tentativa (token de convite reaproveitado ou segunda confirmação de self-service) recebe o mesmo erro genérico. **Achado real da sessão**: o rascunho de ADR-057 escrito por uma sessão anterior — antes da spec ser aprovada pelo PI — tinha removido `POST /activate` do controller (rota do convite por e-mail) sem perceber que a issue exige coexistência; a spec aprovada, publicada no `main` enquanto esta fatia já estava em andamento, corrigiu o rumo e a rota foi restaurada. **CPF e data de nascimento aparecem mascarados** nos campos novos (`mascararCpf`, `mascararDataNascimento`, `src/lib/mascaras.ts`, mobile); os campos que já existiam na aba Perfil (nascimento, aluno desde, plano, unidade) já estavam formatados desde a F23 e não mudaram — CPF não aparece no Perfil hoje, e a spec (§9, resolvida) confirma que os 5 campos são de tela nova (§6.3), não da Home. **Sem migration**: nenhuma tabela nova — `activationRef` não persiste, e `criarOuAtivarConta` reaproveita `student_accounts`. Gate: lint 2/2 (api+mobile), typecheck 2/2, build (api 1/1; `expo export`/Metro do mobile falhavam com `Unable to resolve "@arenahub/ui/app-tokens"` por `packages/ui/dist` incompleto — causa raiz achada nesta fatia: `pnpm --filter @arenahub/ui build` saía 0 sem emitir `.js` nenhum porque um `tsconfig.build.tsbuildinfo` obsoleto convencia o `tsc` de que nada mudou; apagar o `.tsbuildinfo` resolve, e o build normal por `pnpm build`/turbo nunca pisava nisso por já rodar limpo — corrigido para poder abrir o emulador), unitário 1826 (api) + 155 (mobile), integração 1070 (api, suíte inteira, sem crash). **`docs/design/DS-APP.md` §3.1/§4.2/§5.5 atualizado**: CPF substitui e-mail/telefone no diagrama de login (com a nota de retrocompatibilidade), fluxo de dois passos com selo "Cadastro encontrado" e os 5 campos descrito, texto de exemplo do §5.5 trocado. **Verificado no emulador** (Pixel 10 Pro XL, API local + seed `arena-positiva`): login mostra só CPF; "Primeiro acesso?" consulta o aluno de bancada do totem (CPF `00000000191`) e mostra selo "Cadastro encontrado" + nome + os 5 campos formatados; senhas diferentes recusam com a mensagem certa; criar a senha abre a sessão DIRETO, sem pedir login de novo — confirmado também relogando pelo CPF recém-criado. **Um defeito só a tela revelou**: o subtítulo da consulta ("Confirme seus dados de matrícula...") aparecia DUAS VEZES — uma no cabeçalho da folha (`app/entrar.tsx`) e outra dentro do próprio `FormularioDePrimeiroAcesso`, que reimplementava o mesmo texto; nenhum teste via os dois juntos porque o teste do formulário não renderiza o cabeçalho da tela. Corrigido removendo a duplicata do componente (PR #335) |
| F70 | SPEC-070 | 7 | — | Contrato com cláusulas, qualificação das partes e assinatura | [`SPEC-070-contrato-com-clausulas-e-assinatura.md`](specs/SPEC-070-contrato-com-clausulas-e-assinatura.md) · [ADR-055](DECISIONS.md#adr-055) | [#319](https://github.com/RodReis/arenahub/issues/319) | ✅ **entregue** em 11/09/2026 — aguardando aceite. **O PDF deixou de ser ficha de dados e virou contrato de três seções**: Quadro resumo (F63, intacto), Cláusulas (texto versionado por `terms_version`, cravado no fechamento do rascunho — regerar anos depois reproduz o texto de então, a mesma regra dos preços copiados) e Assinaturas (as duas partes e duas testemunhas). **Ativar passou a exigir qualificação completa da CONTRATANTE**: CNPJ, endereço, responsável e CPF do responsável no `Tenant`, mais o foro no próprio contrato — antes o PDF imprimia "não informado" e seguia; agora a ativação é recusada (422) nomeando o campo que falta. **Dados da CONTRATADA (RRB TRADING) vêm de variável de ambiente**, nunca de literal no gerador — todas opcionais, e o campo sem valor sai com `[a definir pelo PI]` visível no papel (decisão do PI: nunca silêncio). **Upload do PDF assinado** por rota própria (`POST .../signed-document`), guardado sob chave separada (`-signed.pdf`) — nunca sobrescreve o gerado; idempotente por estado (`signatureStatus`), recusa segundo upload com 409. **Seis das oito lacunas da spec §7 foram decididas pelo PI em 11/09/2026** e estão escritas no texto: vencimento em 10 dias, multa de 2% e juros de 1% ao mês, suporte por WhatsApp em dias úteis das 9h às 18h, aviso prévio de 30 dias, rescisão por inadimplência acima de 60 dias, teto de responsabilidade nos 12 meses pagos e uso de dado agregado anonimizado mantido. O **foro é a comarca da academia** e vem do próprio contrato (`foro_cidade`/`foro_uf`), não do texto versionado — a Cláusula 13.4 traz um marcador que o gerador troca pela comarca gravada. A **revisão jurídica foi dispensada pelo PI**, que assumiu o risco por escrito de usar texto não revisado por advogado. **Seguem pendentes** apenas os dados cadastrais: endereço e representante da RRB TRADING (variáveis `CONTRATADA_*`) e CNPJ/endereço da Arena Positiva — os dois aparecem como `[a definir pelo PI]` no papel, e o segundo impede ativar contrato novo para aquela academia. A **`2026.1` foi corrigida em vez de substituída por uma `2026.2`**, por decisão do PI: ela só existiu com marcas de pendência e nenhum contrato chegou a ser assinado sob ela, então preencher foi terminar o rascunho, não reescrever o que alguém já assinara — a regra do ADR-055 segue valendo daqui em diante. Gate: lint 10/10 e typecheck 15/15 com `--force`, build 9/9, unitário 3195, integração 1019 sem falha (o `test:integration` isolado crasha no Windows por defeito pré-existente do Jest, não desta fatia — as suítes tocadas rodaram isoladas e passaram 100%). **Seis ajustes pedidos pelo PI depois de ver a tela** (11/09/2026): a grid perdeu o `— sem prazo` (prazo indeterminado é a regra, e repeti-lo em toda linha gastava altura sem distinguir nada), ganhou **Valor do mês** vindo da prévia da fatura (a mesma conta da F64, não uma segunda aritmética), passou a mostrar a **variação corrente do índice** e o aniversário por extenso (`IPCA · 1/1` lia-se como fração) e teve o preço unitário alinhado em grade de duas colunas — com `R$ 3,00 por ativo` numa string só, os valores das duas linhas terminavam em pontos diferentes. No **documento gerado**, entrou a **memória de cálculo** (`1177 × R$ 5,00 = R$ 5.885,00`, parcelas na linha e total embaixo) e a variação corrente do índice. E o **"Corrigir variação" deixou de inserir linha nova**: a competência agora é `readOnly` no modo correção — a API grava por `upsert` em `(code, competencia)`, então trocar o mês ali não corrigia a linha clicada, criava outra competência e deixava a original com o valor errado (`readOnly` e não `disabled`: campo desabilitado não entra no `FormData`). A contagem de alunos saiu de `PlatformInvoiceUseCase` para `contagem-de-alunos.ts` — o contrato passou a precisar dela e injetar a fatura de volta fecharia um ciclo. **Verificado na tela**: grid conferida no navegador, PDF gerado e lido de volta, correção do índice gravando uma linha só (440 → 550, sem inserir 2027). **Com migration** — aplicada localmente via `migrate diff --output` + `psql` (mesmo caminho documentado nas F21/F36/F66/F67) e **aplicada em produção** em 12/09/2026 pelo Pre-Deploy da Railway (`prisma migrate deploy`), confirmada no log do deploy `74adc645`: *"Applying migration `20260911150000_f70_contrato_clausulas_e_assinatura`"*, seguida de *"All migrations have been successfully applied"* e da API subindo sem erro. As cinco variáveis `CONTRATADA_*` foram gravadas no serviço de produção antes do merge, sem redeploy próprio — passaram a valer no reinício do deploy. O **cadastro da Arena Positiva foi gravado em produção** em 12/09/2026 pelo PI, no console do Postgres da Railway (a leitura da credencial do banco foi recusada pelo classificador de segurança, então o SQL saiu pronto e quem executou foi ele): razão social `CENTRO ESPORTIVO ARENA POSITIVA LTDA`, CNPJ, endereço em Trindade/GO, telefone, responsável Douglas dos Reis com CPF e e-mail — `UPDATE 1`, conferido na saída. **O CNPJ anterior em produção não era nulo**, era `48925650000174`, e foi substituído pelo que o PI passou; o banco de desenvolvimento tinha a coluna vazia, e foi de lá que veio a suposição errada de que produção também estaria. Com isso a academia ficou com **qualificação completa** e passa a poder fechar contrato novo |

> **F61–F67 criadas em 08/09/2026 por ADR-052, ADR-053 e ADR-054** — módulo `platform`, **MVP 7 — Plataforma**, fora de `docs/prd/academia/`. `SPEC-061`–`SPEC-067` alocadas aqui pela primeira vez. Ordem sugerida: F61 → F62 → F65 → F66 → F63 → F64 → F67; as pendências do ADR-052 foram fechadas pelo PI no mesmo dia (inativo = todo status ≠ `ACTIVE`; emissão dia 1; preço do inativo negociado por contrato) e as sete specs estão `aprovada-pi`.
