...
| F58 | SPEC-058 | 1 | — | Implantação: API e painel na Railway, totem local, pré-produção | [`SPEC-058-implantacao-nuvem-e-totem-local.md`](specs/SPEC-058-implantacao-nuvem-e-totem-local.md) · [ADR-051](DECISIONS.md#adr-051) | [#253](https://github.com/RodReis/arenahub/issues/253) | 🚧 **em andamento** — branch `feat/f58-implantacao-nuvem-totem-local`. Código pronto: `REDIS_URL` obrigatória em produção, rate limit no login (10 tentativas erradas/min por IP+e-mail), porta 3210 fixa no `start` do kiosk, bootstrap de tenant idempotente. Infra da Railway já provisionada (Postgres, Redis, Bucket, variáveis, pre-deploy de migration, healthcheck) — falta o primeiro deploy real, que espera o PR mesclar. **Não é produção**: restrição 1 do ADR-029 segue em vigor |
