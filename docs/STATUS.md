...
| F26 | SPEC-026 | 4 | 4.4 | Avaliações e consentimentos | [`SPEC-026-avaliacoes-e-consentimentos.md`](specs/SPEC-026-avaliacoes-e-consentimentos.md) | [#26](https://github.com/RodReis/arenahub/issues/26) | ✅ **entregue** em 12/09/2026 — aguardando aceite. Reusa `HealthProgressService`/`AiAnalysisService` (zero regra nova de avaliação); generaliza `ConsentRepository` (era só `'BIOMETRIC'`) para gestão self-service de consentimento no app; reforma `HealthExportService` de síncrono para assíncrono (`ADR-056`, painel e app pelo mesmo caminho). `ConsentDocumentType` ganha `TERMS`/`PRIVACY`/`MARKETING`. `BIOMETRIC` fica fora do self-service (cadastro presencial, revogação exige exclusão física nos leitores — fatia futura). Ver `docs/DEVELOPMENT.md` e `ADR-056` |
